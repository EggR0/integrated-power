...
3. EggR 하네스: 에이전트 라우팅, 실행, 상태·토큰 기록을 담당한다.
4. Antigravity 대시보드: EggR 상태를 읽어 GUI로 보여주고, 사용자가 명시적으로 요청할 때 하네스 번들을 설치·갱신한다.
...
기존 Antigravity `globalStorage` 자료는 삭제하거나 이동하지 않는다. 사용자가 **EggR: Install or Update Antigravity Harness** 명령을 실행하면 알려진 구형 대·소문자 드라이브 해시 위치에서 새 EggR 상태 경로로 없는 파일만 복사한다.
...
0.3.0부터 대시보드 활성화는 하네스를 설치하지 않는다. 명시적 설치 명령만 다음 트랜잭션을 수행한다.
...
이 분리는 대시보드 릴리스와 하네스 릴리스가 나중에 서로 다른 저장소·버전 주기를 갖게 하기 위한 첫 단계다.
...
5. 명령 팔레트에서 **EggR: Install or Update Antigravity Harness**를 한 번 실행한다.
