...
- Output: globalStorage `discussions/` and `sessions/`
...
- Output: globalStorage `reports/codex-<stamp>.md` unless `-OutputFile` is provided
...
`runs.jsonl` is the shared event log for delegated work. The Antigravity IDE dashboard's Open Runs command opens the workspace-specific globalStorage `.agent-runs/runs.jsonl`.
