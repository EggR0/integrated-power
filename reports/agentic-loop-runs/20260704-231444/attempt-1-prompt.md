...
|------|-------|-----|--------|
| C:\Users\jsp0\Documents\Intergrated POWER\scripts\dispatch\Invoke-AgenticLoop.ps1 | 1 | ALL | 16166 | 
...
User Prompt:
Invoke-AgenticLoop.ps1의 KeepArtifacts 기본 ArtifactDir 충돌 문제를 고쳐라.

문제:
현재 -KeepArtifacts 사용 시 ArtifactDir가 비어 있으면 reports\agentic-loop-runs\yyyyMMdd-HHmmss 만 사용한다. 병렬 실행하면 같은 초에 시작한 두 run이 같은 디렉터리와 attempt 파일을 공유해서 prompt/output artifact가 덮어써진다.

요구사항:
1. 기본 ArtifactDir 생성 시 timestamp에 milliseconds를 포함하라. 예: yyyyMMdd-HHmmss-fff
2. 그 뒤에 짧은 GUID suffix도 붙여라. 예: yyyyMMdd-HHmmss-fff-1a2b3c4d
3. 사용자 지정 -ArtifactDir가 있는 경우에는 그대로 존중한다.
4. 다른 동작은 바꾸지 마라.
5. 필요한 SEARCH/REPLACE 블록만 출력하라.
