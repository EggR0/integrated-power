...
2. **Local Agentic Worker**
   - Use for token-efficient local coding edits, test edits, small refactors, and bounded implementation when the target files are explicit.
   - This is the default local route for repository file writes. Do not use raw `Invoke-LocalLLM.ps1` for file edits.
   - Use workspace `scripts/dispatch/Invoke-DelegatedAgentTask.ps1` with `-WorkerBackend Auto`.
   - Prefer `-PromptFile` and `-FilesListFile` when calling from Antigravity or another wrapper so PowerShell array quoting does not corrupt the file list.
   - Do not send destructive tasks such as delete, move, or rename to the local worker unless the user explicitly approves a narrowed file set and the call includes `-AllowDestructive`.
   - Read `references/local-agentic-worker.md`.
...
3. **Local LLM / vLLM**
   ...
   - This is the default delegated route for non-file-writing preprocessing unless the task clearly needs high-end coding judgment or the local machine is unhealthy.
...
- If the user asks local LLM to modify repository files, interpret that as Local Agentic Worker, not raw Local LLM Mode.
