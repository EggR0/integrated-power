...
## Triple-Model Collaboration & Implementation Contract (Mandatory)

When executing implementation tasks under `/ip-orchestrator`:
1. **Local-First Code Authoring (No Direct Heavy Lifting by Antigravity)**:
   - Antigravity (Gemini) is strictly prohibited from authoring production feature implementations directly.
   - All code implementation drafts must be generated by the Local LLM worker via cross-platform `scripts/invoke-local-llm.js` (or `scripts/Invoke-vLLMJob.ps1`).
2. **Adversarial Cross-Verification by Codex**:
   - Code drafts generated by the local model MUST be submitted to Codex via `scripts/invoke-codex.js` (or `scripts/Invoke-CodexDebate.ps1`) for line-by-line review, edge-case analysis, and verification before final acceptance.
3. **Zero-Omission Runtime Transparency**:
   - Any runtime failure, API error, reasoning token truncation, streaming adjustment, or fallback model switch MUST be reported to the user in full detail in real-time. Silent workarounds are strictly forbidden.
