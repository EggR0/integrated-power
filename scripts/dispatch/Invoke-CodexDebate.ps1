...
if ([string]::IsNullOrWhiteSpace($CodexExe) -and $env:CODEX_EXE) {
    $CodexExe = $env:CODEX_EXE
}
...
if ([string]::IsNullOrWhiteSpace($CodexExe)) {
    ...
    if ([string]::IsNullOrWhiteSpace($CodexExe)) {
        throw "Codex executable not found. Set -CodexExe, CODEX_EXE, or install codex.exe on PATH."
    }
}
